...
from StewartPlatformGUI_V4 import Ui_MainWindow
...
class KalmanFilter1D:
    def __init__(self, x_init=0.0, P_init=1.0, Q=0.0001, R=0.05):
        """
...
class StewartApp(QMainWindow):
    def __init__(self):
        ...
        # ================= Force Placement =================
        self.fp_plot_setup()
        self.ui.btn_ForcePlacement_ImportData.clicked.connect(self.fp_import_csv)
        self.ui.btn_ForcePlacement_Calculate.clicked.connect(self.fp_calculate)
        self.ui.btn_ForcePlacement_Reset.clicked.connect(self.fp_reset)
        self.ui.btn_ForcePlacement_ExportData.clicked.connect(self.fp_export)
    ...
    def toggle_connection(self):
        if self.reader is None:
            port_text = self.ui.COMComboBox.currentText().strip()
            if not port_text:
                self.append_output("[Error] No COM port selected.")
                return
...
# ---- Force Placement Tool ----
    def fp_plot_setup(self):
        """
        Initialize Force Placement figures/canvases and insert them into UI widgets.
        """
        # Data placeholders
        self.fp_data = None
        self.fp_csv_path = ""

        # Moment figure and canvas
        self.fp_fig = Figure(figsize=(6, 2))
        self.fp_canvas = FigureCanvasQTAgg(self.fp_fig)
        self.fp_ax_moment = self.fp_fig.add_subplot(111)
        self.fp_ax_moment.set_title("Moment Estimation")
        self.fp_ax_moment.set_ylabel("Moment")
        self.fp_ax_moment.grid(True)

        # Attach to UI widget (create layout if missing)
        plot_area = getattr(self.ui, "widget_ForcePlacement_MomentPlot", None)
        if plot_area is not None:
            if not plot_area.layout():
                plot_area.setLayout(QVBoxLayout())
            plot_area.layout().addWidget(self.fp_canvas)
            plot_area.layout().setContentsMargins(0, 0, 0, 0)

        # xOz figure and canvas
        self.fp_fig_xOz = Figure(figsize=(6, 2))
        self.fp_canvas_xOz = FigureCanvasQTAgg(self.fp_fig_xOz)
        self.fp_ax_xOz = self.fp_fig_xOz.add_subplot(111)
        self.fp_ax_xOz.set_title("Force Position Estimation")
        self.fp_ax_xOz.set_xlabel("x (mm)")
        self.fp_ax_xOz.set_ylabel("z (mm)")
        self.fp_ax_xOz.set_aspect("equal", adjustable="box")
        self.fp_ax_xOz.grid(True)

        plot_area_xoz = getattr(self.ui, "widget_ForcePlacement_xOzPlane", None)
        if plot_area_xoz is not None:
            if not plot_area_xoz.layout():
                plot_area_xoz.setLayout(QVBoxLayout())
            plot_area_xoz.layout().addWidget(self.fp_canvas_xOz)
            plot_area_xoz.layout().setContentsMargins(0, 0, 0, 0)

        # Render initial empty canvases
        self.fp_fig.tight_layout()
        self.fp_fig_xOz.tight_layout()
        self.fp_canvas.draw()
        self.fp_canvas_xOz.draw()

    def fp_import_csv(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Import Force Placement CSV", "", "CSV Files (*.csv)"
        )
        if not path:
            return

        self.fp_csv_path = path
        self.fp_data = pd.read_csv(path)
        # Truncate path if too long
        display_path = path if len(path) <= 50 else "..." + path[-47:]
        self.ui.label_ForcePlacement_PathImportData.setText(display_path)
    def fp_calculate(self):
        if self.fp_data is None:
            return

        # Check if required columns exist
        required_columns = ["Fx_kalman (N)", "Fz_kalman (N)", "My_kalman (Nmm)"]
        missing_columns = [col for col in required_columns if col not in self.fp_data.columns]
        
        if missing_columns:
            self.ui.outputTextEdit.append(f"[Error] Missing columns in CSV: {', '.join(missing_columns)}")
            return

        Fx = self.fp_data["Fx_kalman (N)"].values
        Fz = self.fp_data["Fz_kalman (N)"].values
        M  = self.fp_data["My_kalman (Nmm)"].values

        A = np.column_stack((Fx, Fz))

        p_hat, _, rank, _ = np.linalg.lstsq(A, M, rcond=None)
        if rank < 2:
            self.ui.line_ForcePlacement_deviation.setText("Rank < 2")
            return
        ...
        # Residuals
        residual = M - M_hat
        deviation = np.sqrt(np.mean(residual**2))
        ...
        accuracy = np.sqrt(Cov[0, 0] + Cov[1, 1])

        # Update UI
        self.ui.line_ForcePlacement_x.setText(f"{x_hat:.4f}")
        self.ui.line_ForcePlacement_z.setText(f"{z_hat:.4f}")
        self.ui.line_ForcePlacement_deviation.setText(f"{deviation:.4e}")
        self.ui.line_ForcePlacement_convariance.setText(f"{accuracy:.4e}")

        self.fp_update_plot(Fx, Fz, M, M_hat, x_hat, z_hat, Cov)
    def fp_update_plot(self, Fx, Fz, M, M_hat, x_hat, z_hat, Cov):
        self.fp_fig.clear()
        self.fp_fig_xOz.clear()

        # ---- Moment plot ----
        ax1 = self.fp_fig.add_subplot(111)
        ax1.plot(M, label="Measured Mpitch")
        ax1.plot(M_hat, "--", label="Estimated Mpitch")
        ax1.set_title("Moment Estimation")
        ax1.set_ylabel("Moment")
        ax1.legend(loc ="lower right")
        ax1.grid(True)

        # ---- Geometry plot ----
        ax2 = self.fp_fig_xOz.add_subplot(111)
        ax2.set_title("Exerted Force Position Estimation")
        ax2.plot(0, 0, "ko", label="Origin")
        ax2.plot(x_hat, z_hat, "ro", label="Force point")
        ...
        ax2.plot([x_hat, x_hat], [0, z_hat], "r--", linewidth=1, alpha=0.7)
        ax2.plot([0, x_hat], [z_hat, z_hat], "r--", linewidth=1, alpha=0.7)

        # Draw dimension lines
        # Horizontal dimension (x)
        ax2.plot([0, x_hat], [-0.15, -0.15], "k-", linewidth=1)
        ax2.plot([0, 0], [-0.15, -0.12], "k-", linewidth=1)
        ax2.plot([x_hat, x_hat], [-0.15, -0.12], "k-", linewidth=1)
        ax2.text(x_hat/2, -0.22, f"x = {x_hat:.3f}m", ha="center", fontsize=9)
        
        # Vertical dimension (z)
        ax2.plot([-0.15, -0.15], [0, z_hat], "k-", linewidth=1)
        ax2.plot([-0.15, -0.12], [0, 0], "k-", linewidth=1)
        ax2.plot([-0.15, -0.12], [z_hat, z_hat], "k-", linewidth=1)
        ax2.text(-0.25, z_hat/2, f"z = {z_hat:.3f}m", ha="center", fontsize=9, rotation=90)
        ...
        chi2 = 5.991  # 95%
        width  = 2 * np.sqrt(chi2 * eigvals[0])
        height = 2 * np.sqrt(chi2 * eigvals[1])
        ...
        ax2.set_xlabel("x (mm)")
        x_min, x_max = min(0, x_hat - width/2), max(0, x_hat + width/2)
        z_min, z_max = min(0, z_hat - height/2), max(0, z_hat + height/2)
    ...
    def fp_reset(self):
        ...
        self.ui.label_ForcePlacement_PathImportData.setText("")
        self.ui.line_ForcePlacement_x.clear()
        self.ui.line_ForcePlacement_z.clear()
        self.ui.line_ForcePlacement_deviation.clear()
        self.ui.line_ForcePlacement_convariance.clear()
    ...
    def fp_export(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Result", "", "CSV Files (*.csv)"
        )
        if not path:
            return

        result = {
            "x": [self.ui.line_ForcePlacement_x.text()],
            "z": [self.ui.line_ForcePlacement_z.text()],
            "deviation": [self.ui.line_ForcePlacement_deviation.text()],
            "convariance": [self.ui.line_ForcePlacement_convariance.text()]
        }

        pd.DataFrame(result).to_csv(path, index=False)


## End new application instance
